...
class Map:
    ...
    def get_antinode_pos(self, coord1: Tuple[int], coord2: Tuple[int]) -> List[Tuple[int]]:
        distance = (coord1[0] - coord2[0], coord1[1] - coord2[1])
        antinodes_list = [ coord1, coord2 ] # Antennas are also antinodes as they are in the line
        
        prev_coord1 , prev_coord2 = coord1, coord2
        
        while self.belongs_to_matrix(prev_coord1) or self.belongs_to_matrix(prev_coord2):
            new_calc = [ (prev_coord1[0] + distance[0], prev_coord1[1] + distance[1]), 
    ...
    for i in range(matrix.N_ROWS):
        for j in range(matrix.N_COLS):
            ...
            if not curr_el in checked_freqs:
                checked_freqs.append(curr_el) # Dont check twice the same frequency
