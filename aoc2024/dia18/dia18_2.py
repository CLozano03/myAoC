...
def dijkstra(maze, distances: List[List], start_pos: Tuple[int], end_pos: Tuple[int]) -> int:
...
# Quiting corrupted memory spaces until dijkstra finds a path to reach the goal
back_it = 1
while True:
    i,j = coords[-back_it]; grid[i][j] = '.'
     
    if dijkstra (grid, distances, (0,0), (70,70)) != -1: break
    
    distances: List[List[str]] = [[-1 for _ in range(M_SIZE)] for _ in range(M_SIZE)]
    distances[0][0] = 0
    back_it += 1

print(f'First byte that will prevent the exit from being reachable: {i,j}')