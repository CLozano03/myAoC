...
def swap_wires(_a: str, _b: str) -> None:
    inverted_gates[_a], inverted_gates[_b] = (
        inverted_gates[_b],
        inverted_gates[_a],
    )
    gates[inverted_gates[_a]], gates[inverted_gates[_b]] = (
        gates[inverted_gates[_b]],
        gates[inverted_gates[_a]],
    )
...
def full_adder_check_swapped() -> List[str]:
    ...
    for i in range(45):
        ...
        y_in: str = f"y{i:02}"
        z_out: str = (
            f"z{i:02}"  # Expected register to be the output of the operation
        )

        sum_with_no_carry: str = gates[
            x_in, y_in, XOR
        ]  # Adding in binary is XOR
        carry_bit_reg: str = gates[
            x_in, y_in, AND
        ...
        if prev_carry:
            ...
            if (
                op not in gates
            ):  # If the operation is not in the gates, we need to swap the wires
                in_reg_1, in_reg_2 = list(
                    set(inverted_gates[z_out][:2]) ^ set(op[:2])
                )  # Find the two inputs that are not in the operation
                ...
                gates[op] != z_out
            ):  # If the operation is not the same as the expected output register, we need to swap the wires too
                # Add to the swapping list
            ...
            prev_carry = gates[
                *sorted([prev_carry, sum_with_no_carry]), AND
            ]  # Carry is the AND of the carry bit and the sum with no carry
            prev_carry = gates[
                *sorted([prev_carry, carry_bit_reg]), OR
            ]  # Carry is the AND of the carry bit and the sum with no carry
        else:
...
for op in open(FILENAME).read().split("\n\n")[1].splitlines():
    ...
    in1, in2 = sorted(
        [in1, in2]
    )  # Sort the inputs so it is stored as x01, y01 instead of y01, x01
