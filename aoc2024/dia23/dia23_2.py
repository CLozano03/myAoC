...
def bron_kerbosch(connections: Dict[str, List[str]]) -> List[Set[str]]:
    def bron_kerbosch_rec(R: Set[str], P: Set[str], X: Set[str], cliques: List[Set[str]]):
        '''
        ...
        for node in list(P):
            ...
            bron_kerbosch_rec( R | {node}, P & neighbors, X & neighbors, cliques)
            P.remove(node) # Remove the node from P
            X.add(node)    # Add the node to X so we don't visit it again

    cliques = []
    nodes = set(connections.keys())
    bron_kerbosch_rec(set(), nodes, set(), cliques) # Start finding cliques with an empty R
    return cliques
