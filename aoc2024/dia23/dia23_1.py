...
def interconnected_computers(
    ...
    for combo in combinations(computers, CONNECTIONS_MAX_LEN):
        clique = set(combo)
        if len(clique) != CONNECTIONS_MAX_LEN:
            continue  # Skip if the clique is not the desired length

        # Check if all nodes in the clique are connected between them
        if all(
            other in connections[node]
            for node in clique
            for other in clique
            if other != node
        ):
...
total_starts_with_t = sum(
    1
    for clique in connected
    if any(node.startswith(STARTING_WITH) for node in clique)
)

print(
    f"N of seq of len {CONNECTIONS_MAX_LEN} that at least one starts with '{STARTING_WITH}': {total_starts_with_t}"
)
