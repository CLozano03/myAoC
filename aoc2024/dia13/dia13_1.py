...
class Machine:
    ...
    def solvable(self) -> bool:
        ...
        gcd_1 = math.gcd(self.button_a[0], self.button_b[0])  # Greatest common divisor
        solvable1 = self.prize[0] % gcd_1 == 0
        gcd_2 = math.gcd(self.button_a[1], self.button_b[1])  # Greatest common divisor
...
def calculate_tokens(machine: Machine) -> int:
    ...
    for i in range(101):
        for j in range(101):
            check_x = machine.get_button_a_X() * i + machine.get_button_b_X() * j
            
            if check_x != machine.get_prize_X():
                continue
            
            check_y = machine.get_button_a_Y() * i + machine.get_button_b_Y() * j
