...
class Tree:
    ...
    '''
    def __init__(self):
        self.root = TreeNode('~') # Root node. This char is not part of the alphabet
    ...
    def add_towel(self, towel: str):
        ...
        current_node.children['$'] = TreeNode('$') # '$' Means end of the towel
...
def design_possible(design: str) -> bool:
    ...
    for coincidence in coincidences:
        if design.startswith(coincidence):
            if len(coincidence) == len(design): return True
            else:
...
possible_designs = sum(1 for design in desired_designs if design_possible(design))
