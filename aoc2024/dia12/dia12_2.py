...
class Map:
    ...
    def __dfs_for_region_id(self, coord: Tuple[int], letter: str, region_id: int):
        x, y = coord
        if x not in range(self.n_rows) or y not in range(self.n_cols) or self.matrix[x][y] != letter:
            return
        ...
        if self.matrix[x][y] == letter:
            ...
            for dir in DIRS:
                self.__dfs_for_region_id((x + dir[0], y + dir[1]), letter, region_id)
