...
def is_X_MAS(matrix, coord: Tuple[int, int]):
    ...
    inside_matrix = __check_matrix_pos(top_left) and __check_matrix_pos(top_right) and __check_matrix_pos (bottom_left) and __check_matrix_pos(bottom_right)
    if not inside_matrix:
        return 0
    
    first_axe = (el_matrix(matrix, top_left) == 'M' and el_matrix(matrix, bottom_right) == 'S') or (el_matrix(matrix, top_left) == 'S' and el_matrix(matrix, bottom_right) == 'M')
    second_axe = (el_matrix(matrix, top_right) == 'M' and el_matrix(matrix, bottom_left) == 'S') or (el_matrix(matrix, top_right) == 'S' and el_matrix(matrix, bottom_left) == 'M')
