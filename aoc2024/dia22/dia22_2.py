...
def get_most_bananas(buyers_diffs: List[List[int]], buyers_bananas: List[List[int]]) -> int:
    bananas: Dict[Tuple[int], int] = defaultdict(int) # Store the bananas for each sequence
...
for num in secret_nums:
    ...
    buyers_diffs.append(diffs); buyers_bananas.append(bananas)  
