...
def dijkstra(maze, distances: List[List], start_pos: Tuple[int]) -> int:
    ...
    directions = [(-1, 0, True), (1, 0, True), (0, -1, False), (0, 1, False)]  # Up, Down, Left, Right

    while heap_queue:
        _, (c_x, c_y), dir_is_vertical  = heapq.heappop(heap_queue)
        curr_dist = distances[c_x][c_y]
        
        if grid[c_x][c_y] == end: continue
        ...
        for dx, dy, new_dir in directions:
            ...
            if distances[nx][ny] == -1 or distances[nx][ny] > curr_dist + distance_to_next:
                distances[nx][ny] = curr_dist + distance_to_next
                heapq.heappush(heap_queue, (distances[nx][ny], (nx, ny), new_dir))
