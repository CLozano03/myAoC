...
def dijkstra(
    maze, start_pos: Tuple[int], end_pos: Tuple[int]
) -> Tuple[int, int]:
    heap_queue = [
        (0, start_pos, (0, 1))
    ]  # (distance, position, direction) # DIR starts as EAST
    predecessors: Dict[Tuple[int], Set] = defaultdict(
        lambda: set()
    )  # Dictionary to store the predecessors of each position

    distances = defaultdict(
        lambda: float("inf")
    )  # Dictionary to store the distances to each position (with direction)
    distances[(start_pos[0], start_pos[1], (0, 1))] = (
        0  # Distance to start position is 0
    )
