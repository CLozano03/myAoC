...
@lru_cache(maxsize=None) # Apply memoization to the function so it doesn't recalculate the same values twice
def blink_stone(stone: int, times: int) -> int:
    ...
    if stone == 0:
        ...
        return blink_stone(first_half, times - 1) + blink_stone(second_half, times - 1)
    else:
