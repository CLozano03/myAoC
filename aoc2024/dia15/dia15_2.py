...
def read_file() -> Tuple[Map, List[str]]:
    with open(FILENAME, 'r') as file:
        ...
        new_objects = {"O": "[]", "@": "@.", "#": "##", ".": ".."}
        warehouse = Map([list("".join(new_objects[c] for c in row)) for row in map_str.splitlines()])
...
def execute_movements(warehouse: Map, movements: List[str]) -> None:
    for movement in movements:
        ...
        for moving in to_move:
            ...
            if new_pos in to_move: continue # avoid adding the same position twice
            
            curr_el = warehouse.get_el(new_pos)
            move_allowed = True
            
            if curr_el == '#':
                move_allowed = False; break
            elif curr_el == '[':
        ...
        for mov_x, mov_y in to_move[1:]:
            warehouse.set_el((mov_x + dir_vertical, mov_y + dir_horizontal) , warehouse_copy[mov_x][mov_y])
        
        # Set new player pos after moving
        warehouse.set_el(warehouse.player_pos, '.')
        warehouse.player_pos = (warehouse.player_pos[0] + dir_vertical, warehouse.player_pos[1] + dir_horizontal)
        warehouse.set_el(warehouse.player_pos, '@')
