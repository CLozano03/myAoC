...
def calculate_trailhead_score_from_position(matrix: Map, curr_coord: Tuple[int], height: int, visited: List[Tuple[int]]) -> int:
    ...
    for pos in next_pos_list:
        if matrix.belongs_to_matrix(pos) and height + 1 == matrix.get_el(pos) and pos not in visited:
            # If the position is valid and the height is the next one, then we can go there
            trailhead_sum += calculate_trailhead_score_from_position(matrix, pos, height + 1, visited)
