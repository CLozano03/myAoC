    ...
    for key_origin in KEYPAD.keys():
        for key_dest in KEYPAD.keys():
            if key_origin == key_dest:
                seqs_ret[(key_origin, key_dest)] = ["A"]
                continue
...
def all_paths_for_code(
    code: str, paths: Dict[Tuple[Tuple[int]], List[str]]
) -> List[str]:
    """
    ...
    paths_to_each_codedigit: List[List[str]] = [
        paths[(i, j)] for i, j in zip("A" + code, code)
    ]  # "A" + code to start in A
...
@lru_cache(
    maxsize=None
)  # Cache the results of the function to avoid recomputing
def shortest_seq_len(path: str, n_robots: int) -> int:
    ...
    if n_robots == 1:
        return sum(
            len(DIR_SEQUENCES[(x, y)][0]) for x, y in zip("A" + path, path)
        )
    ...
    for x, y in zip("A" + path, path):
        length += min(
            shortest_seq_len(subseq, n_robots - 1)
...
for code in codes:
    code_paths = all_paths_for_code(
        code, NUMERIC_SEQUENCES
    )  # Get all possible paths for typing the code
    min_length = min(
        shortest_seq_len(path, N_INTER_ROBOTS) for path in code_paths
    )
