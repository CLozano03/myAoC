...
def count_cheatable(maze, path: List[Tuple[int]] , distances: List[List[int]]) -> int:
    count  = 0
    ...
    for square in path:
        ...
        for direction in [(0,1), (1,0), (0,-1), (-1,0)]:
            ...
            if sx < 0 or sx >= M_SIZE or sy < 0 or sy >= M_SIZE: continue

            if maze[nx][ny] != '#': continue
            if maze[sx][sy] != '.' and maze[sx][sy] != 'E': continue
