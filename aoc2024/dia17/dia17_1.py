...
with open(FILENAME) as file:
    ...
    registers: Dict[str, int] = {match[0]: int(match[1]) for match in re.findall(r"Register ([A-Z]): (\d+)", registers_str)}
    program: List[int] = list(map(int, instructions_str.split(' ')[1].replace('\n', '').split(',')))
...
def instruction_3(operand: int):
    ...
    '''
    global ins_pointer
    if registers['A'] == 0: return False # Does nothing if A is 0
    ins_pointer = operand
    return True
...
def get_combo_operand(lit_operand: int):
    if lit_operand >= 0 and lit_operand <= 3: return lit_operand
    if lit_operand == 4: return registers['A']
    if lit_operand == 5: return registers['B']
    if lit_operand == 6: return registers['C']
