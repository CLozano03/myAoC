...
def move_file_entirely(files: List[int], from_pos: int, to_pos: int, num_blocks: int) -> List[int]:
...
def compact_disk(disk: List[int]) -> List[int]:
    ...
    for file_index in range(len(file_sizes) - 1, -1, -1):
        ...
        for i in range(len(compacted_disk)):
            if i >= file_start: break # No need to check beyond the file's starting position
            ...
            if compacted_disk[i] == -1:
                ...
                if free_length >= file_size:
                    compacted_disk = move_file_entirely(compacted_disk, file_start, free_start, file_size)
                    break
            else:
                free_start = -1
                free_length = 0
