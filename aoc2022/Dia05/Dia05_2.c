...
void parse_movements(FILE *f, Stack* stacks[]){
    ...
    /* En esta ocasion necesito un stack auxiliar que voy a ir
    reservando con memoria dinamica  */
    Stack* stack_aux;
    ...
    while(fgets(linea, MAX_LINEA, f) != NULL){
        ...
        /* Repetimos n veces mover a la stack_aux*/
        for(i = 0; i < n; i++){
            /* Sacar nodo de la pila y lo metemos en la pila auxiliar.*/
            push(stack_aux, pop(stacks[st_ini])->data);
        }

        /* n veces de la stack aux a la stack final */
        for(i = 0; i < n; i++){
            push(stacks[st_fin], pop(stack_aux)-> data);
        }
    }
}
