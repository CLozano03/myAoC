...
int handRanker::hand_type(const std::string& hand) const {
    int number_of_J = std::count(hand.begin(), hand.end(), 'J');
    if(number_of_J == 5) return 1;


    for(char c : hand) {
        if (c == 'J') continue;
        if(std::count(hand.begin(), hand.end(), c) + number_of_J == 5) return 1;
        if(std::count(hand.begin(), hand.end(), c) + number_of_J == 4) return 2;
        if(std::count(hand.begin(), hand.end(), c) + number_of_J == 3) {
            for(char d : hand){
                if(d == c || d == 'J') continue;
                if(std::count(hand.begin(), hand.end(), d) + number_of_J == 4) return 2;
                if(std::count(hand.begin(), hand.end(), d) == 2 ) return 3;
            }
            return 4;
        } 
        if(std::count(hand.begin(), hand.end(), c) + number_of_J == 2) {
            for(char d : hand) {
                if(d == c || d == 'J') continue;
                if(std::count(hand.begin(), hand.end(), d) + number_of_J == 4) return 2;
                if(std::count(hand.begin(), hand.end(), d) == 3) return 3;
                if(std::count(hand.begin(), hand.end(), d) + number_of_J == 3) return 4;
                if(number_of_J == 0 && std::count(hand.begin(), hand.end(), d) == 2) return 5;
            }
            return 6;
        }
    }
...
int handRanker::greater_card(const char a, const char b) const {
    const std::string order = "AKQT98765432J"; // Ahora la J es la ultima carta cuando se compara individualmente
